...
clinic_path = "clinic.csv"
feat_path = "dlFeature.csv"

# Place data loading and preprocessing on the cpu
with tf.device('/cpu:0'):
    DL_feat = pd.read_csv(feat_path, header = 0, index_col = 0)
   
    clinic_msag = pd.read_csv(clinic_path, header = 0, index_col = 0)

    Pat_ID = DL_feat.index
    Pat_ID = [str(s) for s in Pat_ID]
    DL_feat.index = Pat_ID
   
    Pat_ID = clinic_msag.index
    Pat_ID = [str(s) for s in Pat_ID]
    
    DL_feat = DL_feat.loc[Pat_ID]
    
    Pat_No_ind = np.array(range(len(DL_feat)), np.int32)
    ...
    tra_feat = DL_feat.iloc[tra_No_ind,:]
    val_feat = DL_feat.iloc[val_No_ind,:]
    test_feat = DL_feat.iloc[test_No_ind,:]
   

    tra_Pat_ID = np.array(tra_msag.index)
    tra_treat = np.array(tra_msag.loc[:, 'treatment'], np.float32)
    tra_time = np.array(tra_msag.loc[:, ['OS.time','PFS.time']], np.float32)
    tra_event = np.array(tra_msag.loc[:, ['OS','PFS']], np.float32)
    tra_FFS_time = np.array(tra_msag.loc[:, 'PFS.time'], np.float32)
    tra_FFS_event = np.array(tra_msag.loc[:, 'PFS'], np.float32)
    ...
    val_FFS_time = np.array(val_msag.loc[:, 'PFS.time'], np.float32)
    val_FFS_event = np.array(val_msag.loc[:, 'PFS'], np.float32)
    ...
    test_FFS_time = np.array(test_msag.loc[:, 'PFS.time'], np.float32)
    test_FFS_event = np.array(test_msag.loc[:, 'PFS'], np.float32)
    test_FFS_event[test_FFS_event<0.0] = 0.0
    deep_features_test = np.array(test_feat, np.float32)

    ind_all = np.array(range(len(tra_msag)))
    ind_0 = ind_all[tra_msag.loc[:,'PFS'] == -1]
    ind_1 = ind_all[tra_msag.loc[:,'PFS'] == 1]
    nn_1 = len(ind_1)
    nn_0 = len(ind_0)
...
def Get_loss(output, s_time, s_event, Pat_ind):
    ...
    PFS_loss_cox = DeepSurv_loss(s_time[:,3], s_event[:,3], Pat_ind[:,3], tf.reduce_max(output,1))
    loss_cox = alpha*PFS_loss_cox + belta*OS_loss_cox 
    return loss_cox 
...
def main():
    ...
    with tf.device('/gpu:0'):
        ...
        fc1_1 = _create_fc_layer(x, feature_num, 'relu', 'specific_layer1_1', keep_prob, w_reg = reg_W)
        fc1_2 = _create_fc_layer(fc1_1, feature_num, 'relu', 'specific_layer1_2', keep_prob, w_reg = tf.contrib.layers.l2_regularizer(scale=0.2))
        ...
        fc2_1 = _create_fc_layer(x, feature_num, 'relu', 'specific_layer2_1', keep_prob, w_reg = tf.contrib.layers.l2_regularizer(scale=2e-2))
        ...
        pred_PFS = tf.py_func(np.max, [output1,1], tf.float32)
        ...
        with tf.Session(config = config) as sess:
            ...
            for epoch in range(num_epochs):
                ...
                for i in range(len(val_treat)):
                    ...
                    Pat_pred = sess.run(pred_PFS, feed_dict = {x: deep_features_val[i,:].reshape(1,clinic_num), 
            ...
            np.save('/OutputPath2/Training_results.npy', np.array(results, np.float32))
