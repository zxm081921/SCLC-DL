...
snapshot_dir = "/OutputPath2/output/checkpoint_path/model_epoch"
...
with tf.device('/cpu:0'):
    ...
    tra_feat = DL_feat1.iloc[tra_No_ind,:]
...
def main():
    with tf.device('/gpu:0'):
        ...
        # model
        fc1_1 = _create_fc_layer(x, feature_num, 'relu', 'shared_layer1_1', keep_prob, w_reg = reg_W)
        fc1_2 = _create_fc_layer(fc1_1, feature_num, 'relu', 'specific_layer1_2', keep_prob, w_reg = reg_W)       
        output1 = _create_fc_layer(fc1_2, num_event-1, 'tanh', 'output_1', use_bias= False, w_reg = reg_W_out)

        fc2_1 = _create_fc_layer(x, feature_num, 'relu', 'specific_layer2_1', keep_prob, w_reg = tf.contrib.layers.l2_regularizer(scale=0.005))
        ...
        with tf.Session(config = config) as sess:
            ...
            for i in range(len(tra_Pat_ID)):
                ...
                Pat_pred, pred_response = sess.run([pred_PFS, output2], feed_dict = {x: tra_feat[i,:].reshape(1,clinic_num), keep_prob: 1.0, treatment: treat.reshape(1,dim_interact_feature)}) 
            ...
            wb = os.path.join(output_path, 'RadSigs.csv')
